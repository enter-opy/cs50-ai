...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in zero_genes:
        if people[person]['mother'] is None:
            ...
            if mother in zero_genes and father in one_gene:
            ...
            if mother in zero_genes and father in two_genes:
    ...
    for person in one_gene:
        if people[person]['mother'] is None:
            ...
            if mother in zero_genes and father in zero_genes:
                ...
                probability = 1 - 0.5 * ((1 - PROBS["mutation"]) + PROBS["mutation"])
            elif mother in two_genes and father in zero_genes:
                probability = 1 - 2 * PROBS["mutation"] * (1 - PROBS["mutation"])
            
            if mother in zero_genes and father in one_gene:
                probability = 1 - 0.5 * PROBS["mutation"] * (1 - PROBS["mutation"])
            elif mother in one_gene and father in one_gene:
                probability = 0.5
            elif mother in two_genes and father in one_gene:
                probability = 1 - 0.5 * PROBS["mutation"] * (1 - PROBS["mutation"])
    ...
    for person in people:
        if person in zero_genes:
            joint_probability *= PROBS["trait"][0][person in have_trait]
        if person in one_gene:
            joint_probability *= PROBS["trait"][1][person in have_trait]
        if person in two_genes:
            joint_probability *= PROBS["trait"][2][person in have_trait]
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        gene_normalization_factor = 1 / sum(probabilities[person]['gene'].values())
        trait_normalization_factor = 1 / sum(probabilities[person]['trait'].values())

        for number_of_genes in probabilities[person]["gene"]:
            probabilities[person]["gene"][number_of_genes] *= gene_normalization_factor

        for have_trait in probabilities[person]["trait"]:
            probabilities[person]["trait"][have_trait] *= trait_normalization_factor
